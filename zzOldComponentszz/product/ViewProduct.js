              ...
              <div className='img_div'>
                ...
                <div className='review' style={{ paddingTop: '10px' }}>
                  ...
                  {reviews && reviews.length === 0 && (
                    <>
                      <div className='alert alert-warning alert_warning_custom' role='alert' data-mdb-color='warning'>
                        {' '}
                        No review added yet{' '}
                      </div>
                    </>
                      ...
                      {isAuthenticated ? (
                        <>
                          <form className='col-sm-6  pt-5' onSubmit={submitRating}>
                            ...
                            <div className='mb-2'>
                              <select className='mdb-select' onChange={(e) => setRating(e.target.value)} value={rating} required>
            ...
            <div className='col-sm-6'>
              <div className='product_desc_wrapper'>
                ...
                <div className='qty_and_addtocart'>
                  <div>
                    <div onClick={decreaseValue} className='value-button' id='decrease' value='Decrease Value'>
                      -
                    </div>
                    <input type='number' id='number' value={qty} readOnly />
                    <div onClick={increaseValue} className='value-button' id='increase' value='Increase Value'>
                      +
                    </div>
                  </div>
                  <button style={{ display: countStock ? 'block' : 'none' }} onClick={handleClickCard} className='addtocart'>
                    Add to cart
                  </button>
                </div>
                <div className='stock'>
                  <hr />
                  <h6>Status: {countStock < 1 ? <span className='text-danger'>Out of Stock</span> : <span className='text-success'>Available</span>}</h6>
                </div>
