    ...
    <>
      ...
      <div className='container ' style={{ paddingLeft: '150px', paddingTop: '100px' }}>
        ...
        <div className='btn_div_button' style={{ display: 'flex', justifyContent: 'right' }}>
          <Link to='/admin/category/create' className='btn btn-default btn-primary '>
            + Create Category
          </Link>
        </div>
            ...
            {categories &&
              categories.map((category, id) => (
                <tr key={id}>
                  ...
                  <td>
                    <i onClick={() => deleteCategory(category._id, category.name)} className='far fa-trash-alt btn-danger' style={{ cursor: 'pointer' }}></i>
                  </td>
                </tr>
              ))}
          </tbody>
