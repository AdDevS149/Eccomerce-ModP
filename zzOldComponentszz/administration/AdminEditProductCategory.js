...
const AdminEditProductCategory = () => {
  ...
  const showSingleCategory = () => {
    axios
      .get(`/api/product/category/show/${id}`)
      .then((res) => {
        if (res) {
  ...
  const handleSubmitCategory = (e) => {
    e.preventDefault();
    axios
      .put(`/api/product/category/update/${id}`, { name })
      .then((res) => {
        if (res) {
          showSingleCategory();
    ...
    <>
      ...
      <div className='container custom_class'>
        ...
        <form className=' col-sm-6 offset-3 pt-5 signup_form ' encType='multipart/form-data'>
          ...
          <button onClick={handleSubmitCategory} type='submit' className='btn btn-primary btn-block mb-4'>
            Update Category
          </button>
        </form>
      </div>
