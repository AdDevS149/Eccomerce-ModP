...
const AddToCart = ({ history }) => {
  ...
  const goToCheckout = () => navigate('/shipping')
        ...
        <div className='row'>
                ...
                <div className='row_loop' key={item.product}>
                  ...
                  <div className='colcart'>
                    <div className='qty_and_addtocart'>
                      <div>
                        <div onClick={() => decreaseValue(item.product, item.quantity)} className='value-button' id='decrease' value='Decrease Value'>
                          -
                        </div>
                        <input type='number' id='number' value={item.quantity} readOnly />
                        <div onClick={() => increaseValue(item.product, item.quantity, item.countStock)} className='value-button' id='increase' value='Increase Value'>
                          +
                        </div>
                      </div>
                    </div>
                  ...
                  <div className='colcart'>
                    <h6>
                      <i onClick={() => removeItemFromCart(item.product)} style={{ color: 'red', cursor: 'pointer' }} className='fa fa-trash' aria-hidden='true'></i>
                    </h6>
                  </div>
                </div>
              ))
          ...
          <div className='col-sm-3'>
            <div className='order_sumary'>
              ...
              <button disabled={cartItems.length === 0} onClick={goToCheckout} className='btn_checkout'>
